...
func New(config *Config) (*OHT, error) {
	logger.New(config.DataDir, config.LogFile, config.Verbosity)

	// Let the database take 3/4 of the max open files (TODO figure out a way to get the actual limit of the open files)
	//const dbCount = 3
	//db.OpenFileLimit = 128 / (dbCount + 1)

	//newdb := config.NewDB
	//if newdb == nil {
	//	newdb = func(path string) (db.Database, error) { return db.NewLDBDatabase(path, config.DatabaseCache) }
	//}
